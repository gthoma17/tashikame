#!/bin/bash
cd "$(dirname "$0")/.."
PR_NUMBER=$(gh pr list --head dev-hoge --state open --json number --jq '.[0].number')
if [ -z "$PR_NUMBER" ]; then
...
if [ "$DECISION" != "APPROVED" ]; then
  ...
  echo "   After the PM passes code review, approve with approve-hoge.command and try again."
  read -n 1
  exit 1
fi
...
if [ "$PR_STATE" = "MERGED" ]; then
  git push origin --force origin/main:refs/heads/dev-hoge
fi
