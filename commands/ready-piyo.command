...
WORKTREE="$HOME/Documents/Claude/Worktrees/$(basename "$REPO_DIR")-piyo"
...
if [ ! -d "$WORKTREE" ]; then
  ...
  git -C "$REPO_DIR" worktree add "$WORKTREE" -B dev-piyo origin/main 2>/dev/null || \
  git -C "$REPO_DIR" worktree add "$WORKTREE" dev-piyo 2>/dev/null
else
  git -C "$REPO_DIR" fetch origin
...
ln -sf "$REPO_DIR/documents/tracks/dev-piyo.md" "$WORKTREE/documents/tracks/dev-piyo.md"

STORY=$(awk '/^## Current Story/{f=1;next} f&&match($0,/#[0-9]+/){print substr($0,RSTART+1,RLENGTH-1);exit}' "$REPO_DIR/documents/tracks/dev-piyo.md")
...
if [ -f "$REPO_DIR/.env" ]; then
  if grep -q '^MITTE_BEAT_TRACK=' "$REPO_DIR/.env"; then
    sed 's/^MITTE_BEAT_TRACK=.*/MITTE_BEAT_TRACK=piyo/' "$REPO_DIR/.env" > "$WORKTREE/.env"
  else
    { cat "$REPO_DIR/.env"; echo "MITTE_BEAT_TRACK=piyo"; } > "$WORKTREE/.env"
  fi
  if [ -n "$STORY" ]; then
